...
def atlas_color_extract(image_path):
    global num
    print(f"Extracting Colors From Atlas: '{image_path}'.")

    tmp0 = image_path.replace('.3dst','')
    output_path = f"{tmp0}.txt"

    existing_colors = set() # Keeps track of existing colors
    
    try: # If color file already exists
    ...
    with open(image_path, "rb") as image_file:
        ...
        for i in range(0, len(argb_data), 4):
            ...
            if rgb_hex not in existing_colors: # Checksum for colors that are already added.
                existing_colors.add(rgb_hex)
                rgb_hex_values.append(rgb_hex)
        
        with open(output_path, "a") as output_file: # Writes output colors to Text file (same name)
            for hex_value in rgb_hex_values:
                output_file.write(hex_value + "\n")


        def search_file1(filename):
        ...
        result1 = search_file1("atlas.items.meta_79954554_0.3dst")

        if result1 and num == 0:
            num += 1 # Stops code from loops more than once
            atlas_color_extract(result1) # Then Item Atlas if First fails
    ...
    while close != True:
        ...
        match option:
            ...
            case "4":
                ...
                result0 = search_file0("atlas.terrain.meta_79954554_0.3dst")
                ...
                if result0:
                    atlas_color_extract(result0) # Attempts to find Terrain Atlas.
                elif result1:
                    atlas_color_extract(result1) # Then Item Atlas if First fails
                else:
                    clear()
