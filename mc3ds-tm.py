...
def addToItemAtlas(itemData, textureImgPath, output_folder):
    ...
    if textureImg.size[0] == 16 and textureImg.size[1] == 16:
        ...
        basewidth = 256
        wpercent = (basewidth/float(itemAtlas.size[0]))
        hsize = int((float(itemAtlas.size[1])*float(wpercent)))
        itemAtlas = itemAtlas.resize((basewidth,hsize), Image.Resampling.LANCZOS)

        # Carga el atlas a la memoria
        itemAtlasPixels = itemAtlas.load()

        # Crea la version con resolucion a la mitad respecto a la resolucion inicial
